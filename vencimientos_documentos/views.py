...
def _obtener_documentos_personal(request):
    ...
    OPTIMIZADO: Filtra directamente en la base de datos en lugar de cargar todos los datos
    y filtrar en Python, mejorando significativamente el rendimiento.
    ...
    try:
        hoy = date.today()
        fecha_limite = hoy + timedelta(days=45)  # Fecha límite: hoy + 45 días
        ...
        # Obtener solo los IDs del personal para optimizar las queries siguientes
        personal_ids = list(personal_query.values_list('personal_id', flat=True))
        
        if not personal_ids:
            return documentos
        
        # Obtener personal con relaciones necesarias solo para los que tienen documentos
        personal_dict = {p.personal_id: p for p in personal_query.select_related('region_id', 'comuna_id')}
    ...
    # OPTIMIZADO: Filtrar por fecha directamente en la base de datos
    ...
    ).select_related('region_id', 'comuna_id')
    
    for personal in personales_con_carnet:
        estado = calcular_estado_vencimiento(personal.fecha_vencimiento_carnet)
        
        # Aplicar filtros (ya filtrado por fecha en DB, pero verificamos días restantes)
    ...
    for licencia in licencias_conducir:
        ...
        try:
            ...
            # Aplicar filtros (ya filtrado por fecha en DB)
            if estado['dias_restantes'] is not None and estado['dias_restantes'] <= 45:
...
def _obtener_documentos_maquinarias(request):
    ...
    # Obtener solo los IDs de los equipos para optimizar las queries siguientes
    equipos_ids = list(equipos_query.values_list('equipo_id', flat=True))
    
    if not equipos_ids:
        return documentos
    
    # OPTIMIZADO: Filtrar documentos por fecha directamente en la base de datos
    # Solo obtener documentos que vencen en los próximos 45 días
