...
Este módulo proporciona decoradores que se pueden usar en las vistas
...
def permission_required_custom(permiso_codigo, raise_exception=True, is_ajax=False):
    ...
    Puede usarse para permisos de modelo (ej: 'app.view_model') o permisos de acción (ej: 'app.custom_action').
            ...
            if require_all:
                ...
                if not tiene_alguno:
                    ...
                    if is_ajax:
                        ...
                        return JsonResponse({
                            'status': 'error',
                            'message': f'No tiene permiso para realizar esta acción. Se requiere al menos uno de: {permisos_str}'
                        }, status=403)
                    raise PermissionDenied(
